...
const notify=(text,level=0,delay=0)=>{const notifyLevel={0:"",1:" -error"};setTimeout(()=>{$("?.xhost__notification-container").innerHTML=`<div class="xhost__notification ${notifyLevel[level]}">${text}</div>`},delay);setTimeout(()=>{$("?.xhost__notification-container").innerHTML=""},delay+5E3)};
