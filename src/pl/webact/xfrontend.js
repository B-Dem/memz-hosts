...
const call_native = () => {
  ...
  var sb = [];
  for (var i = 0; i < ss.length; i++) sb.push(ss.charCodeAt(i));
  sb.push(0);
...
function makeUI() {
  ...
  italic.appendChild(
    document.createTextNode("Click the username to generate a random ID")
  );
...
function fetchAccount(i) {
  ...
  if (!mailbox.account_name) {
    ...
    for (var j = 0; j < 8; j += 3) {
      ...
      b64_id +=
        b64_alphabet[
          ((mailbox.account_id[j] << 4) | (mailbox.account_id[j + 1] >> 4)) & 63
        ];
      b64_id +=
        b64_alphabet[
          ((mailbox.account_id[j + 1] << 2) |
            (mailbox.account_id[j + 2] >> 6)) &
            63
        ];
...
function fetchAccounts() {
  for (var i = 0; i < 16; i++) fetchAccount(i);
}

function activateAccount(idx) {
  mailbox.idx = idx;
  var acc_id = ui[idx].id.value;
  if (acc_id.length != 16) acc_id = "XXXXXXXXXXXXXXXX";
  var binary_id = [];
  for (var i = 7; i >= 0; i--) {
    var digit =
      parseInt(acc_id[2 * i], 16) * 16 + parseInt(acc_id[2 * i + 1], 16);
    if (digit != digit) {
